    ...
    },
{
      ...
      type: "计算题",
      title: "开区间内的极值",
      question: "求函数$f(x) = x^3 - 6x^2 + 9x + 2$的极值",
      essence: "这道题考查利用导数求极值。先求导找驻点，再用一阶或二阶导数判别法确定极值类型",
      explanation: `<strong>第一步：求导数</strong>
...
<strong>第三步：判断极值类型</strong>
- 当$x < 1$时，$f'(x) > 0$，函数递增
- 当$1 < x < 3$时，$f'(x) < 0$，函数递减
- 当$x > 3$时，$f'(x) > 0$，函数递增
...
注意：$f'(x)$ 在 $x = 0$ 处不存在（分母为 0），需要把 $x=0$ 作为可能的极值点讨论

令 $f'(x) = 0$，解得 $\\frac{2}{\\sqrt[3]{x}} - 1 = 0 \\Rightarrow \\sqrt[3]{x} = 2 \\Rightarrow x = 8$

分区间讨论（保留原函数定义域）：
- 当 $x < 0$ 时，$\\sqrt[3]{x} < 0$，故 $\\frac{2}{\\sqrt[3]{x}} - 1 < 0$，$f'(x) < 0$（递减）
- 当 $0 < x < 8$ 时，$0 < \\sqrt[3]{x} < 2$，故 $\\frac{2}{\\sqrt[3]{x}} - 1 > 0$，$f'(x) > 0$（递增）
- 当 $x > 8$ 时，$\\sqrt[3]{x} > 2$，故 $\\frac{2}{\\sqrt[3]{x}} - 1 < 0$，$f'(x) < 0$（递减）

因此：
- $x = 0$ 为不可导点，但通过符号变化可知在 $x=0$ 处函数由减到增，故 $x=0$ 是极小值点，$f(0)=0$
- $x = 8$ 为驻点且由增到减，故 $x=8$ 是极大值点，$f(8)=3\\cdot 4 - 8 = 4$`,
