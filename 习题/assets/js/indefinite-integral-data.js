...
$$= x(\\ln^2 x - 2\\ln x + 2) + C$$`,
    ...
    },
    {
      ...
      type: "计算题",
      title: "基本积分公式",
      question: "70. 求 $\\int \\left(x^2 + \\sin x - \\frac{1}{1+x^2}\\right) dx$",
      explanation: `<strong>第一步：应用积分的线性性质</strong>
$$\\int \\left(x^2 + \\sin x - \\frac{1}{1+x^2}\\right) dx = \\int x^2 dx + \\int \\sin x dx - \\int \\frac{1}{1+x^2} dx$$

<strong>第二步：分别计算各项积分</strong>
$$\\int x^2 dx = \\frac{1}{3}x^3 + C_1$$
$$\\int \\sin x dx = -\\cos x + C_2$$
$$\\int \\frac{1}{1+x^2} dx = \\arctan x + C_3$$

<strong>第三步：合并结果</strong>
$$= \\frac{1}{3}x^3 - \\cos x - \\arctan x + C$$

<strong>说明：</strong>任意常数之和仍为任意常数，因此仅需最后加一个任意常数C即可`,
      answer: "$\\frac{1}{3}x^3 - \\cos x - \\arctan x + C$",
      difficulty: "easy",
      category: "基本积分",
      method: "基本积分公式"
    },
    {
      ...
      type: "计算题",
      title: "指数函数积分",
      question: "71. 求 $\\int 2^x e^x dx$",
      explanation: `<strong>第一步：应用指数运算法则</strong>
$$\\int 2^x e^x dx = \\int (2e)^x dx$$

<strong>第二步：应用指数函数积分公式</strong>
$$\\int a^x dx = \\frac{a^x}{\\ln a} + C$$

<strong>第三步：代入计算</strong>
$$= \\frac{(2e)^x}{\\ln(2e)} + C$$

<strong>第四步：化简对数</strong>
$$\\ln(2e) = \\ln 2 + \\ln e = \\ln 2 + 1 = 1 + \\ln 2$$

<strong>第五步：最终结果</strong>
$$= \\frac{(2e)^x}{1 + \\ln 2} + C$$`,
      answer: "$\\frac{(2e)^x}{1 + \\ln 2} + C$",
      difficulty: "medium",
      category: "基本积分",
    ...
    },
    {
      ...
      type: "计算题",
      title: "三角函数恒等变换积分",
      question: "72. 求 $\\int \\cos^2 \\frac{x}{2} dx$",
      explanation: `<strong>第一步：应用降幂公式</strong>
$$\\cos^2 \\frac{x}{2} = \\frac{1}{2}(1 + \\cos x)$$

<strong>第二步：代入积分</strong>
$$\\int \\cos^2 \\frac{x}{2} dx = \\int \\frac{1}{2} (1 + \\cos x) dx$$

<strong>第三步：提出常数因子</strong>
$$= \\frac{1}{2} \\int (1 + \\cos x) dx$$

<strong>第四步：分别积分</strong>
...
$$= \\frac{1}{2} (x + \\sin x) + C$$`,
    ...
    },
    {
      ...
      type: "应用题",
      title: "物理应用-运动方程",
      question: "73. 已知物体以速度 $v = 2t^2 + 1$ (m/s) 沿 Ox 轴作直线运动，当 t = 1 s 时，物体经过的路程为 3 m，求物体的运动方程。",
      explanation: `<strong>第一步：建立微分方程</strong>
设物体的运动方程为 $x=x(t)$，则 $x'(t)=v=2t^2+1$

<strong>第二步：对速度积分求位移</strong>
$$x(t) = \\int (2t^2 + 1)dt = \\int 2t^2 dt + \\int 1 dt$$

<strong>第三步：计算不定积分</strong>
$$= 2 \\cdot \\frac{t^3}{3} + t + C = \\frac{2}{3}t^3 + t + C$$

<strong>第四步：代入初始条件</strong>
当 $t=1$ 时，$x=3$：
$$3 = \\frac{2}{3} \\times 1^3 + 1 + C = \\frac{2}{3} + 1 + C$$

<strong>第五步：求积分常数</strong>
$$C = 3 - \\frac{2}{3} - 1 = \\frac{4}{3}$$

<strong>第六步：写出运动方程</strong>
$$x(t) = \\frac{2}{3}t^3 + t + \\frac{4}{3}$$`,
      answer: "$x(t) = \\frac{2}{3}t^3 + t + \\frac{4}{3}$",
      difficulty: "medium",
      category: "积分应用",
      method: "初值问题"
    },
    {
      ...
      type: "应用题",
      title: "物理应用-结冰问题",
      question: "74. 若某池塘结冰的速度为 $\\frac{dy}{dt} = k\\sqrt{t}$，其中 y (单位: 厘米)表示冰的厚度，t (单位: 小时)表示时间，k 为正常数。当 t=0 时池塘开始结冰，求结冰厚度 y 与时间 t 的函数关系。",
      explanation: `<strong>第一步：建立微分方程</strong>
$$\\frac{dy}{dt} = k\\sqrt{t}$$

<strong>第二步：对两边积分</strong>
$$y(t) = \\int k \\sqrt{t} dt = k \\int t^{\\frac{1}{2}} dt$$

<strong>第三步：应用幂函数积分公式</strong>
$$= k \\cdot \\frac{t^{\\frac{1}{2}+1}}{\\frac{1}{2}+1} + C = k \\cdot \\frac{t^{\\frac{3}{2}}}{\\frac{3}{2}} + C$$

<strong>第四步：化简</strong>
$$= \\frac{2k}{3} t^{\\frac{3}{2}} + C$$

<strong>第五步：代入初始条件</strong>
当 $t=0$ 时，$y=0$：
$$0 = \\frac{2k}{3} \\times 0^{\\frac{3}{2}} + C$$
$$C = 0$$

<strong>第六步：写出函数关系</strong>
$$y(t) = \\frac{2k}{3} t^{\\frac{3}{2}}$$`,
      answer: "$y(t) = \\frac{2k}{3} t^{\\frac{3}{2}}$",
      difficulty: "medium",
      category: "积分应用",
      method: "初值问题"
    }
