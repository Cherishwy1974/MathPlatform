...
<strong>选项C分析：</strong>
令 $t=\\frac{1}{x}$，则当 $x\\to 0^+$ 时 $t\\to +\\infty$，有
$$\\left(1+\\frac{1}{x}\\right)^x = (1+t)^{1/t} = \\exp\\left(\\frac{\\ln(1+t)}{t}\\right).$$
由于 $\\displaystyle\\lim_{t\\to+\\infty}\\frac{\\ln(1+t)}{t}=0$，故
$$\\lim\\limits_{x\\to 0^+}\\left(1+\\frac{1}{x}\\right)^x = e^0 = 1.$$
当 $x\\to 0^-$ 时，$1+\\frac{1}{x}$ 在靠近0的负区间可能为负数或在定义域外，左极限通常不稳定（不存在或无定义），因此整体极限不存在。
总结：右极限为1，左侧一般不存在，故原极限不存在。
...
},
{
  ...
  type: "间断点类型",
  title: "判断间断点类型",
  question: "判断函数$f(x) = \\frac{x^2 - 1}{x - 1}$在$x = 1$处的间断点类型",
  essence: "这道题考查间断点类型的判断。当函数在某点无定义但左右极限都存在且相等时，该点为可去间断点。",
  explanation: `<strong>分析：</strong>
函数在$x = 1$处无定义（分母为0）
...
<strong>结论：</strong>
左右极限都存在且相等，但函数在该点无定义，所以$x = 1$是可去间断点`,
  answer: "可去间断点",
  difficulty: "easy",
  category: "连续性与间断点",
  method: "间断点判断"
...
因为 $\\displaystyle\\lim\\limits_{x\\to 0^-} f(x) = 1 = \\lim\\limits_{x\\to 0^+} f(x)$，两侧极限相等，

所以 $\\displaystyle\\lim\\limits_{x\\to 0} f(x)$ 存在，且
$$\\displaystyle\\lim\\limits_{x\\to 0} f(x) = 1$$`,
    answer: "$\\displaystyle\\lim\\limits_{x\\to 0} f(x) = 1$",
    difficulty: "medium",
    category: "极限基础运算",
    method: "左右极限法"
  },
  ...
  },
{
    ...
    type: "0/0型-等价无穷小",
    title: "等价无穷小替换",
    question: "求 $\\displaystyle\\lim\\limits_{x\\to 0} \\frac{\\tan x - \\sin x}{x^3}$",
    essence: "利用等价无穷小：$\\tan x - \\sin x = \\sin x(\\frac{1}{\\cos x} - 1) = \\sin x \\cdot \\frac{1-\\cos x}{\\cos x}$",
    explanation: `<strong>方法一：等价无穷小</strong>
当$x \\to 0$时：
$\\tan x - \\sin x = \\sin x \\cdot (\\sec x - 1) = \\sin x \\cdot \\frac{1 - \\cos x}{\\cos x}$

利用等价无穷小：$\\sin x \\sim x$，$1 - \\cos x \\sim \\frac{x^2}{2}$，$\\cos x \\to 1$

$$\\lim\\limits_{x\\to 0} \\frac{\\tan x - \\sin x}{x^3} = \\lim\\limits_{x\\to 0} \\frac{x \\cdot \\frac{x^2}{2} \\cdot 1}{x^3} = \\frac{1}{2}$$`,
    answer: "$\\frac{1}{2}$",
    difficulty: "medium",
    category: "等价无穷小",
    method: "等价无穷小替换"
  },
{
    ...
    type: "∞/∞型",
    title: "分式极限",
    question: "求 $\\displaystyle\\lim\\limits_{x\\to +\\infty} \\frac{3x^2 + 2x - 1}{5x^2 - x + 4}$",
    essence: "分子分母同次幂，极限为最高次项系数之比",
    explanation: `<strong>方法：除以最高次幂</strong>
分子分母同除以$x^2$：

$$\\lim\\limits_{x\\to +\\infty} \\frac{3x^2 + 2x - 1}{5x^2 - x + 4} = \\lim\\limits_{x\\to +\\infty} \\frac{3 + \\frac{2}{x} - \\frac{1}{x^2}}{5 - \\frac{1}{x} + \\frac{4}{x^2}}$$

当$x \\to +\\infty$时，$\\frac{1}{x} \\to 0$，$\\frac{1}{x^2} \\to 0$
